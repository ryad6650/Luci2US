...
from models import GEM_MAX, GRIND_MAX, ROLL_MAX_6, Rune, SubStat
...
class TestApplyPowerUp:
    ...
    def test_all_variants_at_level_12(self):
        ...
        rune = _make_rune(grade="Legendaire", level=0, substats=subs)
        for v in _apply_power_up(rune):
            assert v.level == 12

    def test_max_value_matches_max_roll(self):
        """Sur Legend +0, distribution (4,0,0,0) → sub 0 reçoit 4 × max_roll."""
        subs = [SubStat(type="VIT", value=6), SubStat(type="PV%", value=6),
                SubStat(type="DEF%", value=7), SubStat(type="ATQ%", value=5)]
        rune = _make_rune(grade="Legendaire", level=0, substats=subs)
        variants = _apply_power_up(rune)
        # Trouver la variante avec le plus gros VIT (= 6 + 4×6 = 30)
...
class TestProjectToPlus12:
    ...
    def test_hero_4th_sub_gets_max_roll_value(self):
        """Les nouvelles subs ajoutées ont initial = 1 × max_roll."""
        subs = [SubStat(type="VIT", value=6),
                SubStat(type="PV%", value=6),
                SubStat(type="DEF%", value=7)]
        rune = _make_rune(grade="Heroique", level=0, substats=subs)
        ...
        # Au moins une variante où la 4ème sub est CC à valeur 6 (max_roll CC)
        found = False
        for v in variants:
            for s in v.substats:
                if s.type not in original_types and s.type == "CC":
                    if s.value == ROLL_MAX_6["CC"]:
                        found = True
        assert found, "Aucune variante avec CC@6 en 4ème sub"
